...
void srv_connect(const int socket, request_t *request) {
    ...
    /* set up send message request and reply */
    reply_t send_msg_reply;
    ...
    /* read pending messages, send them out and delete them */
    while (db_get_pend_msg(&pend_msg_entry) == DBMS_SUCCESS) {
        ...
        /* send message */
        clt_send_message(&send_msg_request, &send_msg_reply, &entry);
        ...
        if (send_msg_reply.server_error_code == SRV_SUCCESS) {
            ...
            db_io_op_usr_ent(&entry, MODIFY);
            reply.server_error_code = SRV_ERR_SEND_ANY;

            /* sending has failed, can't continue at this point */
            break;
        }
    }
}
...
void srv_send(const int socket, request_t *request) {
    ...
    if (recipient_entry.user.status == STATUS_CN) {
        ...
        /* server log message if SEND MESSAGE succeeds */
        if (send_msg_reply.server_error_code == SRV_SUCCESS) {
            printf("s> SEND MESSAGE %u FROM %s TO %s\n", msg_entry.msg.id, request->message.sender, request->recipient);
            fflush(stdout);
        } else {
            /* if sending fails, change recipient user's status to disconnected */
