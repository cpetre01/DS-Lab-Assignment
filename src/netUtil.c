...
int send_server_reply(const int socket, const reply_t *reply) {
    ...
    CHECK_SOCK_ERROR(write_bytes(socket, (const char *) &reply->server_error_code, 1), GEN_ERR_ANY, socket)
    return 0;
}
...
int send_string(const int socket, const char *string) {
    ...
    int ret_val;    /* needed for error-checking macros */
    CHECK_SOCK_ERROR(write_bytes(socket, string, (int) (strlen(string) + 1)), GEN_ERR_ANY, socket)
    return 0;
}
...
int recv_string(const int socket, char *string) {
    ...
    int ret_val;    /* needed for error-checking macros */
    CHECK_SOCK_ERROR(read_line(socket, string, MAX_MSG_SIZE), GEN_ERR_ANY, socket)
    return 0;
}
